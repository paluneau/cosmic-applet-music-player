...
use cosmic::{Element, theme, widget::Slider, widget::Column, widget::Container};
...
pub fn get_album_cover<'a>(app: &'a CosmicAppletMusic) -> Container<'a, Message, cosmic::Theme> {
    let content = if let Some(ref handle) = app.album_art_handle {
        ...
        .width(cosmic::iced::Length::Fixed(COVER_SIZE))
        .height(cosmic::iced::Length::Fixed(COVER_SIZE))
...
fn view_controls_tab(app: &CosmicAppletMusic, space_s: f32, space_m: f32) -> Element<'_, Message> {
    ...
    // Album cover
    let album_cover = get_album_cover(app);
